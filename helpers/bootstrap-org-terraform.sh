...
echo "Applying permissions for org $ORG_ID and project $SEED_PROJECT..."
 # Grant roles/resourcemanager.organizationAdmin to the Seed Service Account on the organization
 # Allows the Terraform service account to set IAM policies for the Organization
gcloud organizations add-iam-policy-binding \
 ...
 # Allows the Terraform service account to read the organization details.
gcloud organizations add-iam-policy-binding \
  "${ORG_ID}" \
  --member="serviceAccount:${SA_ID}" \
...
# Allows this Terraform Service Account to create projects inside organization
...
# This is necessary for assigning projects to billing accounts
...
# Grant roles/resourcemanager.folderAdmin to the service account on the organization.
# Lets Terraform SA set up and manage folders
echo "Adding role roles/resourcemanager.folderAdmin..."
gcloud organizations add-iam-policy-binding \
  "${ORG_ID}" \
  --member="serviceAccount:${SA_ID}" \
...
# Grants the Service Account the ability to enable APIs
# This is particularly useful for setting up other Terraform-management projects
echo "Adding role roles/serviceusage.serviceUsageAdmin..."
...
# Manages IAM policies. Only on the Seed Project itself
...
# Manages Storage IAM policies. Only on the Seed Project itself
...
\ \ role: roles/billing.admin" policy-tmp-$$.yml && rm policy-tmp-$$.yml.bak
    gcloud beta billing accounts set-iam-policy "$BILLING_ACCOUNT" policy-tmp-$$.yml
  else
    echo "Could not set roles/billing.admin on service account $SERVICE_ACCOUNT.\
